(function() {
    ...
    $('#projects-list').innerHTML = projs.map(p => `
      <article class="project-item">
        ...
        <div class="project-links">
          ${p.link ? projectLinkIcon(p.linkLabel || 'Link', p.link) : ''}
          ${p.github ? projectLinkIcon('GitHub', p.github) : ''}
          ${p.demo ? projectLinkIcon('Demo', p.demo) : ''}
        </div>
      </article>
    `).join('');
  }
  ...
  const PROJECT_ICONS = {
    ...
    'VS Code Marketplace': `<svg width="16" height="16" viewBox="0 0 100 100" fill="currentColor"><path d="M74.9 5.4 51.7 27.2 21.8 3 5.4 14.6l25 21.6L5.4 57.7 21.8 69l30-24.1 23.1 21.7L94.6 55V16.5zm0 16.4V57L56.8 36.9z"/></svg>`,
    'Vercel':              `<svg width="16" height="16" viewBox="0 0 116 100" fill="currentColor"><path d="M57.5 0L115 100H0z"/></svg>`,
  };

  function projectLinkIcon(label, url, fallback) {
    const icon = PROJECT_ICONS[label] || PROJECT_ICONS[fallback] || null;
    return icon
      ? `<a href="${esc(url)}" target="_blank" rel="noopener" class="project-icon-link" title="${esc(label)}">${icon}</a>`
